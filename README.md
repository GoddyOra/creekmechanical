...
tools — calculators, reference data, a WebAssembly STEP/IGES viewer, and a
CAD compatibility checker. Every tool runs entirely client-side; nothing a
...
Stages 0–6 are built: positioning pages (About, Privacy, Terms, Contact),
...
tool pages, and a CAD compatibility checker (unit consistency, open-shell/
non-manifold/orientation checks, assembly structure) with a downloadable
report. `/guides/` and `/services/` are still placeholders; the parametric
part generator and tolerance stack-up tool have not started. See
